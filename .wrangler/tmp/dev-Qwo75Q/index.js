...
// .wrangler/tmp/bundle-l2pIjO/checked-fetch.js
...
var trackConditions = sqliteTable("track_conditions", {
  id: text("id").primaryKey(),
...
var races = sqliteTable("races", {
  ...
  trackCond: text("track_cond", { enum: ["\u826F", "\u7A0D", "\u91CD", "\u4E0D\u826F"] }).notNull(),
  trackConditionId: text("track_condition_id").references(() => trackConditions.id),
  // 外部キー（NULL許容）
...
var raceResults = sqliteTable("race_results", {
  id: text("id").primaryKey(),
...
var raceEntries = sqliteTable("race_entries", {
...
app.post("/api/horses", async (c) => {
  try {
    const db = createDb(c.env.DB);
    const horsesData = await c.req.json();
...
app.post("/api/races", async (c) => {
  try {
    const db = createDb(c.env.DB);
    const racesData = await c.req.json();
...
app.get("/api/race-results", async (c) => {
  ...
  const query = db.select().from(raceResults).where(whereClause).orderBy(desc(raceResults.date), raceResults.popularity);
...
app.post("/api/race-results-with-horses", async (c) => {
  try {
    ...
    const { results } = await c.req.json();
    const resultsData = results;
    const raceMap = /* @__PURE__ */ new Map();
    for (const resultData of resultsData) {
      if (resultData.raceId && !raceMap.has(resultData.raceId)) {
        const raceData = {
          raceId: resultData.raceId,
          date: resultData.date,
          venue: resultData.venueNormalized || resultData.venue,
          // 正規化された開催地
          meetingNumber: resultData.meetingNumber || 1,
          dayNumber: resultData.dayNumber || 1,
          raceNo: resultData.raceNo || 1,
          raceName: resultData.raceName,
          className: resultData.className || "\u672A\u52DD\u5229",
          surface: resultData.courseType,
          distance: resultData.distance,
          direction: resultData.direction || "\u53F3",
          trackCond: resultData.courseCondition,
          fieldSize: resultData.fieldSize,
          win5: false,
          status: "\u78BA\u5B9A"
        };
    ...
    for (const resultData of resultsData) {
      if (resultData.horseId && !horseMap.has(resultData.horseId)) {
        const horseData = {
          ...
          name: resultData.horseName || resultData.raceName.split("\u30FB")[1] || resultData.raceName,
          // 馬名を優先
          birthDate: resultData.horseBirthDate || (resultData.horseAge ? `${new Date(resultData.date).getFullYear() - parseInt(resultData.horseAge) + 1}0101`.padStart(8, "0") : `${new Date(resultData.date).getFullYear() - 2}0101`.padStart(8, "0")),
          // 年齢から生年を計算
          sex: resultData.horseSex || "\u725D",
          // 性別を優先
          color: "",
          father: resultData.horseFather || "",
          mother: resultData.horseMother || "",
          trainer: resultData.horseTrainer || resultData.jockey || "",
          // 調教師を優先
          owner: resultData.horseOwner || "",
          breeder: resultData.horseBreeder || "",
          earnings: resultData.horseEarnings || 0
          // 賞金を優先
        };
...
app.post("/api/race-entries", async (c) => {
  try {
    const db = createDb(c.env.DB);
    const entriesData = await c.req.json();
    for (const e of entriesData) {
      try {
        await db.insert(raceEntries).values({
          raceId: e.raceId,
          horseId: e.horseId,
          date: e.date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0].replace(/-/g, ""),
          frameNo: e.frameNo,
          horseNo: e.horseNo,
          age: e.age,
          jockey: e.jockey,
    ...
    for (const row of csvData) {
      ...
      if (normalizedDate && normalizedDate.length === 6) {
        ...
        if (parts.length >= 3) {
          ...
          const day2 = parts[2].padStart(2, "0");
          normalizedDate = `${year2}${month}${day2}`;
        }
      }
      ...
      const raceData = {
        ...
        raceName: row["\u30EC\u30FC\u30B9\u540D"],
        className: row["\u30EC\u30FC\u30B9\u540D"].includes("\u672A\u52DD\u5229") ? "\u672A\u52DD\u5229" : row["\u30EC\u30FC\u30B9\u540D"].includes("1\u52DD") ? "1\u52DD\u30AF\u30E9\u30B9" : row["\u30EC\u30FC\u30B9\u540D"].includes("2\u52DD") ? "2\u52DD\u30AF\u30E9\u30B9" : row["\u30EC\u30FC\u30B9\u540D"].includes("3\u52DD") ? "3\u52DD\u30AF\u30E9\u30B9" : "OP",
      ...
      const horseData = {
        id: horseId,
        name: row["\u99AC\u540D"],
        birthDate: `${(/* @__PURE__ */ new Date()).getFullYear() - parseInt(row["\u5E74\u9F62"]) + 1}0101`,
        // 年齢から生年を推定
        sex: row["\u6027\u5225"],
        color: row["\u6BDB\u8272"] || "",
        father: row["\u7A2E\u7261\u99AC"] || "",
...
app.get("/api/races/:raceId/entries", async (c) => {
  try {
    ...
    const horsesRows = await db.select().from(horses).where(inArray(horses.id, horseIds));
    const horseMap = new Map(horsesRows.map((h) => [h.id, h]));
...
function getVenueFromRaceId(raceId) {
...
__name(getVenueFromRaceId, "getVenueFromRaceId");
function getMeetingNumberFromRaceId(raceId) {
  return parseInt(raceId.substring(6, 8));
}
__name(getMeetingNumberFromRaceId, "getMeetingNumberFromRaceId");
function getDayNumberFromRaceId(raceId) {
  return parseInt(raceId.substring(8, 10));
}
__name(getDayNumberFromRaceId, "getDayNumberFromRaceId");
function getRaceNoFromRaceId(raceId) {
  const raceNo = parseInt(raceId.substring(raceId.length - 2));
  return raceNo;
}
__name(getRaceNoFromRaceId, "getRaceNoFromRaceId");
...
app.patch("/api/races/:raceId", async (c) => {
  try {
    ...
    const updateFields = { updatedAt: sql`CURRENT_TIMESTAMP` };
    if (updateData.weather !== void 0) updateFields.weather = updateData.weather;
    if (updateData.trackCond !== void 0) updateFields.trackCond = updateData.trackCond;
    if (updateData.status !== void 0) updateFields.status = updateData.status;
    if (updateData.offAt !== void 0) updateFields.offAt = updateData.offAt;
    const result = await db.update(races).set(updateFields).where(eq(races.raceId, raceId));
...
app.patch("/api/horses/:horseId", async (c) => {
  try {
    ...
    const updateData = await c.req.json();
    const updateFields = { updatedAt: sql`CURRENT_TIMESTAMP` };
    if (updateData.currentRaceId !== void 0) updateFields.currentRaceId = updateData.currentRaceId;
    if (updateData.trainer !== void 0) updateFields.trainer = updateData.trainer;
    if (updateData.owner !== void 0) updateFields.owner = updateData.owner;
    if (updateData.earnings !== void 0) updateFields.earnings = updateData.earnings;
    const result = await db.update(horses).set(updateFields).where(eq(horses.id, horseId));
...
app.patch("/api/races/:raceId/cushion", async (c) => {
  try {
    ...
    await db.insert(trackConditions).values({
      id: trackConditionId,
      date: race.date,
      venue: race.venue,
...
app.get("/api/venues", async (c) => {
  try {
    ...
    }).from(races).leftJoin(trackConditions, eq(races.trackConditionId, trackConditions.id)).where(eq(races.date, date));
    ...
    for (const race of raceData) {
      if (!venueMap.has(race.venue)) {
        venueMap.set(race.venue, {
          ...
          track: {
            turf: race.surface === "\u829D" ? race.trackCond : void 0,
            dirt: race.surface === "\u30C0\u30FC\u30C8" ? race.trackCond : void 0
          },
...
app.get("/api/venues/dates", async (c) => {
  try {
    const db = createDb(c.env.DB);
    const dates = await db.select({ date: raceEntries.date }).from(raceEntries).groupBy(raceEntries.date);
    const validDates = dates.map((d) => d.date).filter((dateStr) => {
      ...
      return date instanceof Date && !isNaN(date.getTime()) && date.getFullYear() === year && date.getMonth() === month - 1 && date.getDate() === day;
    }).map((dateStr) => {
      let fullDateStr = dateStr;
      if (dateStr.length === 6) {
        fullDateStr = `20${dateStr}`;
...
// .wrangler/tmp/bundle-l2pIjO/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-l2pIjO/middleware-loader.entry.ts
